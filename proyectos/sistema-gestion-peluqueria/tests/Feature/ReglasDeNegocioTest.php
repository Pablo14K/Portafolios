...
class ReglasDeNegocioTest extends TestCase
{
    ...
    public function test_la_foto_de_perfil_se_carga_y_se_saca(): void
    {
        ...
        // Y el fondo de las iniciales se pinta: la clase que lo apaga no está.
        $this->assertStringNotContainsString('tiene-img', $sinFoto,
            'Sin foto el avatar lleva su fondo de color: `tiene-img` es sólo para cuando hay imagen.');
        ...
        try {
            ...
            $this->assertStringContainsString('sgp-avatar tiene-img', $conFoto,
                'Con foto, el avatar de la barra apaga el fondo de color: si no, se ve debajo mientras la foto carga.');
    ...
    /**
     * El salón elige UN color y el resto se ajusta solo.
     *
     * Es la pantalla que pidió el usuario con su maqueta —Configuración →
     * Ajustes desde la 7.125.0—, y lo que tiene que quedar fijo son tres cosas
     * que se rompen distinto:
     *
     * **Una** — elegir una paleta cambia la pantalla ENTERA y no sólo el botón:
     * el fondo, las tarjetas, el texto, los bordes y las barras salen del mismo
     * color. Si algún token se quedara sin derivar, el sistema saldría con un
     * fondo de un color y los bordes de otro, y eso no se ve desde el código.
     * Y el color es lo ÚNICO que decide los colores: el «ajuste fino» de la
     * 7.124.0 se retiró, así que un token mandado a mano en el POST no pisa
     * nada — si pisara, sería una función escondida que nadie ve en pantalla.
     *
     * **Dos** — el texto tiene tamaño Y tipo de letra, y los dos llegan a la
     * página entera: el tamaño mueve la base del documento y el tipo va en
     * `body`, que es de donde heredan los campos y los botones. Sin lo segundo
     * la pantalla mostraría la opción y el sistema seguiría con la de siempre.
     *
     * **Tres** — restablecer devuelve el verde agua y deja de emitir el bloque.
     *
     * Y de paso: la pantalla es Ajustes y no Sucursales, que la cargaba encima
     * de la lista de locales.
     */
    #[Test]
    public function el_salon_elige_un_color_y_el_resto_se_ajusta_solo(): void
    {
        $this->entrarComo('admin', 'admin123');
        Config::olvidar();

        // De fábrica no se emite ni un byte de más.
        $panel = (string) $this->get(route('panel'))->assertOk()->getContent();
        $this->assertStringNotContainsString('--acento:#', $panel,
            'Sin personalizar, el layout no tiene por qué escribir ninguna variable.');

        // La pantalla ofrece el catálogo, el color propio, el tamaño y el tipo
        // de letra, en Ajustes — y Sucursales dejó de tenerla.
        $html = (string) $this->get(route('seguridad.ajustes'))->assertOk()->getContent();
        foreach (['name="paleta" value="lavanda"', 'name="color_primario"', 'name="letra" value="muy_grande"',
                  'name="fuente" value="clasica"', 'data-ap-previa', 'data-ap-reglas', 'data-ap-fuentes'] as $pieza) {
            $this->assertStringContainsString($pieza, $html,
                "Falta «{$pieza}»: la pantalla de ajustes quedó a medias.");
        }
        $this->assertStringNotContainsString('name="color_acento"', $html,
            'El ajuste fino se retiró (7.125.0): no tiene que quedar ni un campo suyo.');
        $sucursales = (string) $this->get(route('seguridad.sucursales'))->assertOk()->getContent();
        $this->assertStringNotContainsString('data-ap', $sucursales,
            'Los ajustes del sistema salieron de Sucursales: ahí queda la lista de locales y nada más.');

        $lavanda = Tema::catalogo()['lavanda'][1];

        // ---- Una: se elige la paleta y se deriva todo -------------------
        $this->post(route('seguridad.ajustes.guardar'), [
            'nombre_salon' => Config::nombreSalon(),
            'paleta' => 'lavanda',
            'letra' => 'grande',
            'fuente' => 'clasica',
            // El campo del ajuste fino de la 7.124.0: ya no existe, y mandado
            // igual en el POST no tiene que pisar nada.
            'color_acento' => '#112233',
        ])->assertRedirect(route('seguridad.ajustes'));
        Config::olvidar();

        $t = Config::tema();
        $this->assertSame($lavanda, $t['primario']);
        $this->assertSame('lavanda', $t['paleta']);
        $this->assertSame('grande', $t['letra']);
        $this->assertSame('clasica', $t['fuente']);
        $this->assertSame($lavanda, $t['tokens']['acento'],
            'Un color mandado a mano no pisa la paleta: el ajuste fino se retiró.');

        // **El resto salió del mismo tono, no sólo el acento.** La tolerancia
        // es ancha a propósito: el fondo es casi blanco, y ahí un escalón de
        // redondeo del hexadecimal mueve el tono varios grados sin que se vea.
        // Lo que se mide es que sea el tono ELEGIDO y no el de antes, que están
        // a más de cien grados de distancia.
        $fabrica = Tema::paleta(Tema::PRIMARIO);
        $tono = fn (string $hex) => (int) round(Tema::hsl($hex)[0]);
        foreach (['fondo', 'superficie', 'texto', 'borde', 'sup_oscura', 'fondo_oscuro'] as $token) {
            $this->assertEqualsWithDelta($tono($lavanda), $tono($t['tokens'][$token]), 12,
                "«{$token}» no salió del color elegido: el resto NO se ajustó solo.");
            $this->assertNotSame($fabrica[$token], $t['tokens'][$token],
                "«{$token}» se quedó con el verde agua de fábrica.");
        }

        // Y la pantalla entera cambia, no sólo un botón.
        $panel = (string) $this->get(route('panel'))->assertOk()->getContent();
        foreach (['--acento:' . $lavanda, '--fondo:' . $t['tokens']['fondo'],
                  '--sup-oscura:' . $t['tokens']['sup_oscura']] as $v) {
            $this->assertStringContainsString($v, $panel, "El layout no escribió «{$v}».");
        }
        $this->assertStringContainsString('html{font-size:112.5%', $panel,
            'La letra grande escala el documento entero: `app.css` mide todo en `rem`.');
        $this->assertStringContainsString('body{font-family:' . Tema::FUENTES['clasica'][1], $panel,
            'El tipo de letra elegido tiene que llegar a `body`, que es de donde heredan campos y botones.');

        // ---- Dos: sólo el texto, con los colores de fábrica --------------
        // El bloque de colores NO se emite: un salón que sólo agrandó la letra
        // no tiene por qué recibir cuarenta variables con los valores de
        // `app.css`. Y el tamaño nuevo, el más grande, escala a 125 %.
        $this->post(route('seguridad.ajustes.guardar'), [
            'nombre_salon' => Config::nombreSalon(),
            'paleta' => 'agua',
            'letra' => 'muy_grande',
        ...
        $t = Config::tema();
        $this->assertSame(Tema::PRIMARIO, $t['primario']);
        $this->assertSame('muy_grande', $t['letra']);
        $this->assertSame('amplia', $t['fuente']);
        $this->assertTrue(Config::temaPersonalizado(), 'Cambiar sólo la letra ya es personalizar.');
        $panel = (string) $this->get(route('panel'))->assertOk()->getContent();
        $this->assertStringContainsString('html{font-size:125%', $panel);
        $this->assertStringContainsString('body{font-family:' . Tema::FUENTES['amplia'][1], $panel);
        $this->assertStringNotContainsString('--acento:#', $panel,
            'Con el verde agua puesto, el bloque de colores no se escribe aunque la letra sí cambie.');

        // Un tamaño o un tipo inventados caen en los de fábrica, sin
        // rechazar el guardado entero.
        $this->post(route('seguridad.ajustes.guardar'), [
            'nombre_salon' => Config::nombreSalon(),
            'paleta' => 'agua',
            'letra' => 'gigante',
        ...
        Config::olvidar();
        $this->assertSame('normal', Config::letra());
        $this->assertSame('sistema', Config::fuente());
        $panel = (string) $this->get(route('panel'))->assertOk()->getContent();
        $this->assertStringNotContainsString('html{font-size:', $panel, 'La letra volvió a normal.');
        $this->assertStringNotContainsString('body{font-family:', $panel, 'La fuente volvió a la del sistema.');

        // ---- Tres: restablecer devuelve el verde agua --------------------
        $this->post(route('seguridad.ajustes.guardar'), [
            'nombre_salon' => Config::nombreSalon(), 'paleta' => 'lavanda', 'letra' => 'grande', 'fuente' => 'clasica',
        ])->assertRedirect(route('seguridad.ajustes'));
        $this->post(route('seguridad.ajustes.restablecer'))->assertRedirect(route('seguridad.ajustes'));
        Config::olvidar();

        $this->assertFalse(Config::temaPersonalizado());
        $this->assertSame(Tema::PRIMARIO, Config::tema()['primario']);
        $panel = (string) $this->get(route('panel'))->assertOk()->getContent();
        $this->assertStringNotContainsString('--acento:#', $panel,
            'Restablecido, el bloque de colores no se dibuja: vuelve lo que trae `app.css`.');
        $this->assertStringNotContainsString('body{font-family:', $panel);
    }

    /**
     * La paleta derivada se lee: contraste AA en los dos temas.
     *
     * El salón elige un color y no tiene por qué saber que un texto pide 4,5:1
     * ni que el contorno de un campo pide 3:1. **Eso lo tiene que garantizar la
     * derivación**, o la pantalla de personalizar sirve para dejar el sistema
     * ilegible — que es justo lo que la identidad accesible de la 7.123.0 vino
     * a evitar.
     */
    #[Test]
    public function cualquier_color_que_elija_el_salon_sigue_siendo_legible(): void
    {
        $malas = [];
        // Los dieciocho del catálogo, más dos extremos que nadie cargaría de
        // buena fe: un amarillo claro y un negro. Si la derivación se sostiene
        // ahí, se sostiene con cualquier cosa.
        $colores = array_column(Tema::catalogo(), 1);
        $colores[] = '#FFD700';
        $colores[] = '#000000';

        foreach ($colores as $c) {
            $t = Tema::paleta($c);
            $medir = function (string $a, string $b, float $min, string $que) use (&$malas, $c, $t) {
                $v = Tema::contraste($t[$a], $t[$b]);
                if ($v < $min) {
                    $malas[] = sprintf('%s · %s: %.1f:1 (pide %.1f)', $c, $que, $v, $min);
                }
            };

            $medir('texto', 'superficie', 4.5, 'texto sobre la tarjeta');
            $medir('texto_tenue', 'superficie', 4.5, 'texto secundario sobre la tarjeta');
            $medir('sobre_acento', 'acento', 4.5, 'texto encima del botón');
            $medir('borde_control', 'superficie', 3.0, 'contorno de un campo');
            $medir('sobre_oscura', 'sup_oscura', 4.5, 'texto de la barra superior');
            $medir('texto_oscuro', 'superficie_oscuro', 4.5, 'texto sobre la tarjeta, en oscuro');
            $medir('sobre_acento_oscuro', 'acento_oscuro', 4.5, 'texto encima del botón, en oscuro');
        }

        $this->assertSame([], $malas,
            "Hay colores que dejan el sistema por debajo de AA:\n  " . implode("\n  ", $malas));
    }

    // =================================================================
    //  7.127.0 — la compra refleja la factura del proveedor, la seña avisa
    //  y manda su recibo, la agenda no ofrece antes de hora
    // =================================================================

    /**
     * Arma una compra confirmada por la pantalla, con lo que la 7.127.0 sumó.
     *
     * @param  list<array{nombre:string,cantidad:float,precio:float,iva:int}>  $lineas
     * @param  list<array{fecha:string,monto:float}>  $cuotas  vacío = contado
     * @return int el id de la compra
     */
    private function compraPorPantalla(array $lineas, float $descuento = 0.0, array $cuotas = []): int
    {
        $prov = (int) DB::scalar('SELECT id_proveedor FROM proveedor WHERE activo = 1 ORDER BY id_proveedor LIMIT 1');
        $this->assertGreaterThan(0, $prov, 'Premisa: hace falta un proveedor activo.');
        $cat = (int) DB::scalar('SELECT MIN(id_categoria) FROM categoria_producto');
        $condicion = $cuotas
            ? (int) DB::scalar('SELECT id_condicion_venta FROM condicion_venta WHERE dias_credito > 0 AND activo = 1 LIMIT 1')
            : (int) DB::scalar('SELECT id_condicion_venta FROM condicion_venta WHERE dias_credito = 0 AND activo = 1 LIMIT 1');
        $this->assertGreaterThan(0, $condicion, 'Premisa: hacen falta las dos condiciones de compra.');

        $antes = (int) DB::scalar('SELECT COALESCE(MAX(id_compra), 0) FROM compra');
        $post = [
            'id_proveedor' => $prov,
            'id_condicion_venta' => $condicion,
            'nro_factura_proveedor' => '001-001-0007127',
            'descuento' => monto_input($descuento),
            'nombre' => array_column($lineas, 'nombre'),
            'id_producto' => array_fill(0, count($lineas), 0),
            'cantidad' => array_map(fn ($l) => cant($l['cantidad']), $lineas),
            'precio' => array_map(fn ($l) => monto_input($l['precio']), $lineas),
            'tasa_iva' => array_column($lineas, 'iva'),
            'categoria' => array_fill(0, count($lineas), $cat),
            'cuota_fecha' => array_column($cuotas, 'fecha'),
            'cuota_monto' => array_map(fn ($c) => monto_input($c['monto']), $cuotas),
        ];
        $this->post(route('inventario.compra.guardar'), $post)->assertRedirect();

        $id = (int) DB::scalar('SELECT MAX(id_compra) FROM compra');
        $this->assertGreaterThan($antes, $id, 'La compra tiene que haberse guardado: ' . json_encode(session('sgp_flash')));

        return $id;
    }

    /**
     * La compra refleja la factura del proveedor: descuento, IVA por renglón y
     * la cuota que vence.
     *
     * Reportado por el usuario sobre el pago al proveedor: «no se refleja que
     * la compra se hizo a crédito, sólo muestra el monto total, no el de la
     * cuota», y «también debe mostrar descuento (IVA posiblemente también), ya
     * que es un reflejo de la factura de una compra». Y detrás había un
     * defecto: `fn_compra_vencimiento` devolvía la PRIMERA cuota aunque ya
     * estuviera pagada, así que una compra en cuotas con la primera al día
     * seguía «vencida» para siempre.
     *
     * Se comprueba con la cuenta entera: dos renglones con IVA distinto y un
     * descuento sobre el comprobante, en dos cuotas con la primera ya vencida.
     */
    #[Test]
    public function la_compra_refleja_la_factura_del_proveedor_con_descuento_iva_y_la_cuota_que_vence(): void
    {
        $this->entrarComo('admin', 'admin123');

        $ayer = date('Y-m-d', strtotime('-1 day'));
        $enUnMes = date('Y-m-d', strtotime('+30 days'));
        $id = $this->compraPorPantalla(
            [['nombre' => 'Shampoo prueba 7127', 'cantidad' => 2, 'precio' => 100000, 'iva' => 10],
             ['nombre' => 'Guantes prueba 7127', 'cantidad' => 1, 'precio' => 50000, 'iva' => 0]],
            25000,
            [['fecha' => $ayer, 'monto' => 100000], ['fecha' => $enUnMes, 'monto' => 125000]]
        );

        // Lo que dice la base: bruto, descuento, total, y el IVA incluido con
        // el descuento repartido — la misma cuenta que la factura de venta.
        $v = DB::selectOne('SELECT * FROM vw_cuenta_proveedor WHERE id_compra = ?', [$id]);
        $this->assertNotNull($v, 'La compra confirmada tiene que estar en Cuentas por pagar.');
        $this->assertSame(250000.0, (float) $v->bruto);
        $this->assertSame(25000.0, (float) $v->descuento);
        $this->assertSame(225000.0, (float) $v->total, 'El total es los renglones menos el descuento del comprobante.');
        $this->assertSame(225000.0, (float) $v->saldo);
        $this->assertSame(2, (int) $v->cuotas);

        $iva = DB::selectOne('SELECT * FROM vw_compra_impuestos WHERE id_compra = ?', [$id]);
        // 200.000 gravado al 10 % y 50.000 exento, con el 10 % de descuento
        // repartido: 180.000 y 45.000; el IVA es 180.000 / 11.
        $this->assertSame(180000.0, (float) $iva->gravado_10);
        $this->assertSame(45000.0, (float) $iva->exentas);
        $this->assertEqualsWithDelta(16363.64, (float) $iva->iva_10, 0.01);
        $this->assertSame(10, (int) DB::scalar(
            "SELECT dc.tasa_iva FROM detalle_compra dc JOIN producto p ON p.id_producto = dc.id_producto
              WHERE dc.id_compra = ? AND p.nombre = 'Shampoo prueba 7127'", [$id]),
            'El IVA queda en el RENGLÓN, como en detalle_factura.');
        $this->assertSame(0, (int) DB::scalar(
            "SELECT dc.tasa_iva FROM detalle_compra dc JOIN producto p ON p.id_producto = dc.id_producto
              WHERE dc.id_compra = ? AND p.nombre = 'Guantes prueba 7127'", [$id]));

        // **La cuota que vence es la 1ª**, vencida ayer, y el pago propone su
        // monto y no el saldo entero.
        $this->assertSame(1, (int) $v->cuota_pendiente);
        $this->assertSame(100000.0, (float) $v->cuota_falta);
        $this->assertSame($ayer, (string) $v->vencimiento);
        $this->assertSame(1, (int) $v->vencida);

        $html = (string) $this->get(route('facturacion.proveedores'))->assertOk()->getContent();
        $modal = substr($html, strpos($html, 'id="modalPago' . $id . '"'));
        $modal = substr($modal, 0, strpos($modal, '</form>'));
        $this->assertStringContainsString('en 2 cuotas', $html, 'La fila dice que es a crédito y en cuántas cuotas.');
        $this->assertStringContainsString('name="monto" data-min="0"', $modal);
        $this->assertStringContainsString('value="' . monto_input(100000) . '"', $modal,
            'El pago propone la cuota que vence, no el saldo entero.');
        $this->assertStringContainsString('Descuento de la factura', $modal, 'Y muestra el descuento del comprobante.');
        $this->assertStringContainsString('IVA incluido', $modal, 'Y el IVA.');

        // Se paga la 1ª: la que vence pasa a ser la 2ª, y la compra deja de
        // estar vencida aunque la 1ª tuviera fecha de ayer.
        Bd::idDe('sp_pagar_compra', [$id, 1, 1, 100000, 'prueba', null]);
        $v = DB::selectOne('SELECT * FROM vw_cuenta_proveedor WHERE id_compra = ?', [$id]);
        $this->assertSame(2, (int) $v->cuota_pendiente, 'Pagada la 1ª, la que sigue es la 2ª.');
        $this->assertSame(125000.0, (float) $v->cuota_falta);
        $this->assertSame($enUnMes, (string) $v->vencimiento,
            'El vencimiento es el de la cuota pendiente, no el de la primera: con eso la compra dejaba de estar vencida.');
        $this->assertSame(0, (int) $v->vencida);

        // Un pago que cubre una cuota y media: la 2ª queda «en parte».
        Bd::idDe('sp_pagar_compra', [$id, 1, 1, 25000, 'prueba', null]);
        $cuotas = \App\Servicios\Compras::cuotas($id);
        $this->assertSame('pagada', $cuotas[0]->estado);
        $this->assertSame('parcial', $cuotas[1]->estado);
        $this->assertSame(100000.0, (float) $cuotas[1]->falta);

        // Y el descuento no puede pasarse de los renglones: la pantalla lo rechaza.
        $antes = (int) DB::scalar('SELECT COUNT(*) FROM compra');
        $this->post(route('inventario.compra.guardar'), [
            'id_proveedor' => (int) DB::scalar('SELECT id_proveedor FROM compra WHERE id_compra = ?', [$id]),
            'id_condicion_venta' => 1,
            'descuento' => monto_input(999999),
            'nombre' => ['Otro prueba 7127'], 'id_producto' => [0], 'cantidad' => ['1'],
            'precio' => [monto_input(1000)], 'tasa_iva' => [10],
            'categoria' => [(int) DB::scalar('SELECT MIN(id_categoria) FROM categoria_producto')],
        ])->assertRedirect(route('inventario.compra_form'));
        $this->assertSame($antes, (int) DB::scalar('SELECT COUNT(*) FROM compra'),
            'Un descuento mayor que los renglones no es un descuento.');
    }

    /**
     * La nota de crédito del proveedor baja el saldo de la compra, y no es un pago.
     *
     * Pedido del usuario: «una compra en sí es una factura, por lo que debe
     * poder adjuntar su nota de crédito correspondiente». Se comprueba el
     * ciclo entero —registrarla con el documento adjunto, que descuente, que
     * no cuente como pagado, que no pueda pasarse del saldo, que el archivo
     * se sirva y que anularla devuelva la deuda—, en las dos direcciones.
     */
    #[Test]
    public function la_nota_de_credito_del_proveedor_baja_el_saldo_de_la_compra_y_no_es_un_pago(): void
    {
        $this->entrarComo('admin', 'admin123');
        $id = $this->compraPorPantalla(
            [['nombre' => 'Tintura prueba NC', 'cantidad' => 3, 'precio' => 60000, 'iva' => 10]]
        );
        $this->assertSame(180000.0, (float) DB::scalar('SELECT fn_compra_saldo(?)', [$id]));

        // Por más de lo que se debe, no: sería un crédito a favor sin dueño.
        $this->post(route('inventario.compra.nota_credito'), [
            'id_compra' => $id, 'nro_documento' => '001-001-0000900', 'fecha' => date('Y-m-d'),
            'monto' => monto_input(200000), 'motivo' => 'Devolución de tres cajas rotas',
        ])->assertRedirect(route('inventario.compra_ver', ['id' => $id]));
        $this->assertSame(0, (int) DB::scalar('SELECT COUNT(*) FROM compra_nota_credito WHERE id_compra = ?', [$id]),
            'Una nota por más del saldo se rechaza.');

        // Con el papel adjunto: una imagen de verdad, que se mira por contenido.
        // Sin GD en el contenedor no hay `fake()->image()`: va un PNG de 1×1
        // escrito a mano, que es lo que el respaldo mira (el contenido, no la
        // extensión).
        $archivo = UploadedFile::fake()->createWithContent('nota.png', base64_decode(
            'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChwGA60e6kgAAAABJRU5ErkJggg=='));
        $this->post(route('inventario.compra.nota_credito'), [
            'id_compra' => $id, 'nro_documento' => '001-001-0000901', 'fecha' => date('Y-m-d'),
            'monto' => monto_input(60000), 'motivo' => 'Devolución de una caja rota',
            'archivo' => $archivo,
        ])->assertRedirect(route('inventario.compra_ver', ['id' => $id]));

        $nota = DB::selectOne('SELECT * FROM compra_nota_credito WHERE id_compra = ?', [$id]);
        $this->assertNotNull($nota, 'La nota tiene que quedar registrada: ' . json_encode(session('sgp_flash')));
        $this->assertNotEmpty($nota->archivo, 'Con el documento adjunto.');
        $this->assertSame(60000.0, (float) DB::scalar('SELECT fn_compra_acreditado(?)', [$id]));
        $this->assertSame(120000.0, (float) DB::scalar('SELECT fn_compra_saldo(?)', [$id]),
            'La nota baja el saldo…');
        $this->assertSame(0.0, (float) DB::scalar('SELECT fn_compra_pagado(?)', [$id]),
            '…pero NO es un pago: no salió de ninguna caja.');

        // La ficha la muestra, y sirve el documento
        $html = (string) $this->get(route('inventario.compra_ver', ['id' => $id]))->assertOk()->getContent();
        $this->assertStringContainsString('001-001-0000901', $html);
        $this->assertStringContainsString('sgp-compra-notas', $html);
        $this->get(route('inventario.compra.nota_credito.archivo', ['id' => (int) $nota->id_nota]))
            ->assertOk()->assertHeader('Content-Type', 'image/png');

        // Y Cuentas por pagar lo dice
        $v = DB::selectOne('SELECT * FROM vw_cuenta_proveedor WHERE id_compra = ?', [$id]);
        $this->assertSame(60000.0, (float) $v->acreditado);
        $this->assertSame(120000.0, (float) $v->saldo);

        // Con el saldo bajado, un pago por el saldo viejo se rechaza en la base
        try {
            Bd::idDe('sp_pagar_compra', [$id, 1, 1, 180000, 'prueba', null]);
            $this->fail('La base tiene que topar el pago contra el saldo ya acreditado.');
        } catch (\PDOException $e) {
            // `Bd::idDe` habla con PDO directo: lo que levanta es PDOException
            $this->assertStringContainsString('supera el saldo', $e->getMessage());
        }

        // Anular devuelve la deuda, y la nota se queda en la ficha
        $this->post(route('inventario.compra.nota_credito.anular'), [
            'id_nota' => (int) $nota->id_nota, 'motivo' => 'Se cargó dos veces por error',
        ])->assertRedirect(route('inventario.compra_ver', ['id' => $id]));
        $this->assertSame(180000.0, (float) DB::scalar('SELECT fn_compra_saldo(?)', [$id]));
        $this->assertSame(1, (int) DB::scalar('SELECT COUNT(*) FROM compra_nota_credito WHERE id_compra = ? AND activo = 0', [$id]),
            'Anular no borra.');

        @unlink(storage_path('app/respaldos/' . $nota->archivo));
    }

    /**
     * Los últimos registros de Asistencia paginan.
     *
     * Reportado por el usuario: «el registro de asistencia (tabla) no tiene
     * paginación». Cortaba con `LIMIT 60` sin decirlo. Se garantiza la
     * premisa —más filas que una página— y se mide que la segunda exista.
     */
    #[Test]
    public function los_ultimos_registros_de_asistencia_paginan(): void
    {
        $this->entrarComo('admin', 'admin123');
        $suc = (int) session('id_sucursal');

        $ut = DB::selectOne(
            'SELECT ut.id_usuario, ut.id_turno FROM usuario_turno ut
               JOIN turno_laboral t ON t.id_turno = ut.id_turno
              WHERE t.id_sucursal = ? AND t.activo = 1 ORDER BY ut.id_turno LIMIT 1', [$suc]);
        $this->assertNotNull($ut, 'Premisa: alguien con turno en este local.');

        // Treinta días hacia atrás, sin pisar lo que ya haya
        $insertadas = 0;
        for ($i = 400; $i < 460; $i++) {
            $fecha = date('Y-m-d', strtotime("-$i days"));
            $insertadas += DB::affectingStatement(
                'INSERT IGNORE INTO asistencia (id_turno, id_usuario, fecha, hora_entrada) VALUES (?,?,?,?)',
                [(int) $ut->id_turno, (int) $ut->id_usuario, $fecha, '08:0' . ($i % 10) . ':00']);
        }
        $this->assertGreaterThan(25, $insertadas, 'Premisa: más filas que una página.');

        // Abrir la pantalla materializa las faltas de hoy cuya tolerancia
        // venció, así que el total se cuenta DESPUÉS, no antes.
        $html = (string) $this->get(route('seguridad.asistencia'))->assertOk()->getContent();
        $total = (int) DB::scalar(
            'SELECT COUNT(*) FROM asistencia a JOIN turno_laboral t ON t.id_turno = a.id_turno WHERE t.id_sucursal = ?', [$suc]);
        $this->assertStringContainsString('Mostrando <strong>1–25</strong>', $html,
            'La primera página dice hasta dónde llega.');
        $this->assertStringContainsString('de <strong>' . $total . '</strong>', $html,
            'Y cuántos hay en total: es lo que el LIMIT 60 escondía.');

        $pag2 = (string) $this->get(route('seguridad.asistencia', ['p' => 2]))->assertOk()->getContent();
        $this->assertStringContainsString('Mostrando <strong>26–', $pag2, 'La segunda página existe.');
    }

    /**
     * La agenda no ofrece atender, poner en proceso ni marcar ausente antes de hora.
     *
     * Reportado por el usuario: «en agenda aparecen botones que no deberían
     * mostrarse antes, como registrar atención». El servidor rechazaba
     * atender desde la 7.82.0 y la fila lo ofrecía igual; «En proceso» y
     * «Ausente» ni siquiera se rechazaban. Se mide la fila y el servidor, en
     * las dos direcciones: con la cita a tres horas no están y el POST se
     * rechaza; con la hora pasada, sí están.
     */
    #[Test]
    public function la_agenda_no_ofrece_atender_ni_ausente_antes_de_hora_y_el_servidor_tampoco_lo_acepta(): void
    {
        $this->entrarComo('admin', 'admin123');

        // En un local sin otras citas, para que la fila que se mide sea la única
        $suc = $this->otraSucursal();
        $this->conSucursal($suc);
        $hora = (int) ahora_bd('H');
        $futura = $hora <= 20
            ? date('Y-m-d H:i:00', strtotime(ahora_bd() . ' +3 hours'))
            : date('Y-m-d H:i:00', strtotime(ahora_bd() . ' +25 minutes'));
        $this->assertSame(ahora_bd('Y-m-d'), substr($futura, 0, 10), 'Premisa: la cita es de HOY y todavía falta.');

        $srv = (int) DB::scalar('SELECT id_servicio FROM servicio WHERE activo = 1 ORDER BY id_servicio LIMIT 1');
        $cliente = (int) DB::scalar('SELECT id_cliente FROM cliente WHERE activo = 1 ORDER BY id_cliente LIMIT 1');
        DB::insert('INSERT INTO cita (id_cliente, id_usuario, id_sucursal, fecha_hora, id_estado_cita) VALUES (?,?,?,?,1)',
            [$cliente, 1, $suc, $futura]);
        $id = (int) DB::scalar('SELECT LAST_INSERT_ID()');
        DB::insert('INSERT INTO cita_servicio (id_cita, id_servicio) VALUES (?,?)', [$id, $srv]);

        $fila = function () use ($id): string {
            $html = (string) $this->get(route('citas.agenda', ['dia' => ahora_bd('Y-m-d')]))->assertOk()->getContent();
            $i = strpos($html, 'data-bs-target="#detCita' . $id . '"');
            $this->assertNotFalse($i, 'La cita tiene que estar en la agenda del día.');
            $desde = strrpos(substr($html, 0, $i), '<tr');

            return substr($html, $desde, strpos($html, '</tr>', $i) - $desde);
        };

        $f = $fila();
        $this->assertStringNotContainsString('title="Registrar atención"', $f, 'A tres horas no se atiende.');
        $this->assertStringNotContainsString('title="En proceso"', $f, 'Ni se pone en proceso.');
        $this->assertStringNotContainsString('title="Ausente"', $f, 'Ni se da por ausente a quien todavía puede venir.');
        $this->assertStringContainsString('title="Cancelar"', $f, 'Cancelar sí, desde siempre.');
        $this->assertStringContainsString('title="Reprogramar"', $f, 'Y reprogramar.');

        // Esconder no es el control: el POST armado a mano se rechaza
        foreach ([5, 6] as $estado) {
            $this->post(route('citas.estado'), ['id_cita' => $id, 'id_estado_cita' => $estado, 'dia' => ahora_bd('Y-m-d')])
                ->assertRedirect();
            $this->assertSame(1, (int) DB::scalar('SELECT id_estado_cita FROM cita WHERE id_cita = ?', [$id]),
                'El estado ' . $estado . ' no se acepta antes de hora.');
        }

        // Con la hora pasada, vuelven
        DB::update('UPDATE cita SET fecha_hora = ? WHERE id_cita = ?',
            [date('Y-m-d H:i:00', strtotime(ahora_bd() . ' -10 minutes')), $id]);
        $f = $fila();
        $this->assertStringContainsString('title="Ausente"', $f, 'Pasada la hora, se puede dar por ausente.');
        $this->assertTrue(
            str_contains($f, 'title="Registrar atención"') || str_contains($f, 'falta fichaje') || str_contains($f, 'profesional ausente'),
            'Llegada la hora, se ofrece atender — o se dice qué lo traba.');
    }

    /**
     * La campanita avisa la seña que la clienta registró, y sólo a quien cobra.
     *
     * Pedido del usuario: «validar si llegó la seña para cobrarla — hacer que
     * haya un aviso». La solicitud del portal se veía sólo en la fila de la
     * agenda de ese día; la de la semana que viene no la veía nadie, y la cita
     * se soltaba al vencer el plazo con la clienta convencida de que pagó.
     */
    #[Test]
    public function la_campanita_avisa_la_sena_que_la_clienta_registro_y_solo_a_quien_cobra(): void
    {
        $this->entrarComo('admin', 'admin123');
        $cita = $this->citaFuturaAgendada((int) session('id_sucursal'));

        DB::insert('INSERT INTO sena_solicitud (id_cita, monto) VALUES (?, 50000)', [$cita->id_cita]);
        $idSol = (int) DB::scalar('SELECT LAST_INSERT_ID()');

        $senas = array_values(array_filter(Alertas::mias(), fn ($a) => $a['clave'] === 'sena:' . $idSol));
        $this->assertCount(1, $senas, 'La seña por confirmar tiene que estar en la campanita.');
        $this->assertStringContainsString('50.000', $senas[0]['que']);
        $this->assertStringContainsString(fecha($cita->fecha_hora, 'd/m'), $senas[0]['que'], 'Con el día de la cita.');
        $this->assertSame('facturacion.cobros', $senas[0]['permiso'], 'A quien cobra: es quien confirma.');
        $this->assertStringContainsString('cobrar=' . $cita->id_cita, $senas[0]['url'],
            'El enlace abre la agenda con la ventana de cobro de ESA cita.');
        $this->assertStringContainsString('dia=' . $cita->dia, $senas[0]['url'], 'Y en el día de la cita, no hoy.');

        // La barra lo dibuja con su enlace propio
        $html = (string) $this->get(route('panel'))->assertOk()->getContent();
        $this->assertStringContainsString('cobrar=' . $cita->id_cita, $html);

        // Confirmada, el aviso se va solo
        DB::update('UPDATE sena_solicitud SET id_cobro = (SELECT MIN(id_cobro) FROM cobro) WHERE id_solicitud = ?', [$idSol]);
        $this->assertSame([], array_values(array_filter(Alertas::mias(), fn ($a) => $a['clave'] === 'sena:' . $idSol)),
            'Confirmada la seña, no hay nada que avisar.');

        // A quien no cobra, nada
        DB::update('UPDATE sena_solicitud SET id_cobro = NULL WHERE id_solicitud = ?', [$idSol]);
        DB::delete("DELETE FROM rol_modulo WHERE id_rol = 2 AND modulo IN ('facturacion','facturacion.cobros')");
        DB::update('UPDATE usuario SET id_rol = 2 WHERE id_usuario = 1');
        session(['rol' => 2]);
        Permisos::olvidar();
        $this->assertSame([], array_values(array_filter(Alertas::mias(), fn ($a) => $a['clave'] === 'sena:' . $idSol)),
            'Quien no cobra no puede confirmarla: avisarle es ruido.');
    }

    /**
     * Al confirmar la seña, la clienta recibe el recibo con la confirmación.
     *
     * Pedido del usuario: «con el pago de la seña la clienta debe recibir un
     * recibo con el aviso de confirmación de cita». Se mide que el correo
     * salga a SU dirección, que diga que la cita quedó confirmada cuando la
     * seña cubre lo que el salón pide, que lleve el recibo en PDF, y que quede
     * anotado en la cola de avisos como enviado.
     */
    #[Test]
    public function al_confirmar_la_sena_la_clienta_recibe_el_recibo_con_la_confirmacion(): void
    {
        Mail::fake();
        $this->entrarComo('admin', 'admin123');
        $suc = (int) session('id_sucursal');
        ...
        // La clienta con correo, y un servicio que pida seña: así la seña
        // confirma de verdad y el correo tiene que decirlo.
        DB::update('UPDATE persona pe JOIN cliente cl ON cl.id_persona = pe.id_persona
                       SET pe.email = ? WHERE cl.id_cliente = ?', ['clienta7127@example.com', $cita->id_cliente]);
        DB::update('UPDATE servicio s JOIN cita_servicio cs ON cs.id_servicio = s.id_servicio
                       SET s.sena_porcentaje = 50 WHERE cs.id_cita = ?', [$cita->id_cita]);
        $requerida = (float) DB::scalar('SELECT fn_cita_sena_requerida(?)', [$cita->id_cita]);
        $this->assertGreaterThan(0, $requerida, 'Premisa: la cita pide seña.');

        if (! DB::scalar('SELECT COUNT(*) FROM caja WHERE id_estado_caja = 1 AND id_sucursal = ?', [$suc])) {
            Caja::abrir(1, 0.0, $this->cajonDe($suc));
        }
        // `Caja::abierta()` cachea por proceso, y otra prueba de la misma
        // corrida pudo dejarla en null parada en un local sin caja.
        Caja::olvidar();

        $this->post(route('facturacion.sena'), [
            'id_cita' => $cita->id_cita, 'dia' => $cita->dia,
            'metodo' => [1], 'monto' => [monto_input($requerida)],
        ])->assertRedirect(route('citas.agenda', ['dia' => $cita->dia]));

        $this->assertSame($requerida, (float) DB::scalar('SELECT fn_cita_sena(?)', [$cita->id_cita]),
            'Premisa: la seña entró — ' . json_encode(session('sgp_flash')));

        Mail::assertSent(ReciboSena::class, function (ReciboSena $m) use ($cita) {
            $this->assertTrue($m->hasTo('clienta7127@example.com'), 'A la clienta.');
            $this->assertStringContainsString('queda confirmada', $m->envelope()->subject,
                'Con la seña completa, el asunto dice que la cita quedó confirmada.');
            $this->assertTrue($m->r['confirmada']);
            $adj = $m->attachments();
            $this->assertCount(1, $adj, 'Con el recibo en PDF adjunto.');
            $html = $m->render();
            $this->assertStringContainsString('Tu cita quedó confirmada', $html);
            $this->assertStringContainsString(fecha($cita->fecha_hora, 'd/m/Y'), $html);
            $this->assertStringContainsString('Queda por pagar al terminar', $html);

            return true;
        });

        $this->assertSame(1, (int) DB::scalar(
            "SELECT COUNT(*) FROM notificacion WHERE id_cita = ? AND id_tipo_notificacion = 2 AND estado = 'ENVIADA'",
            [$cita->id_cita]), 'Y queda anotado en la cola como enviado: el registro de lo que se le mandó está entero.');

        // El PDF se dibuja de verdad
        $r = \App\Servicios\Sena::recibo($cita->id_cita, [(int) DB::scalar(
            'SELECT MAX(id_cobro) FROM cobro WHERE id_cita = ?', [$cita->id_cita])]);
        $pdf = new \Dompdf\Dompdf();
        $pdf->loadHtml(view('correo.recibo_sena_pdf', ['r' => $r])->render(), 'UTF-8');
        $pdf->render();
        $this->assertStringStartsWith('%PDF-', (string) $pdf->output());
    }
}
