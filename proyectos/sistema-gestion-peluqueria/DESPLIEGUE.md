...
|---|---|
| Reescribir los **84 `DEFINER`**, o error 1449 en la pantalla de ingreso | **No hace falta**: adentro del contenedor se importa y se consulta como root |
...
> quien sólo quiere probar el sistema — ver *Los archivos de entorno* en `CLAUDE.md`.
...
- **Los 84 `DEFINER`.** Las rutinas se crearon con ``DEFINER=`root`@`localhost` ``. Importadas
  ...
  llame a una función — o sea en la pantalla de ingreso. Se resuelve reescribiendo el `.sql`
  antes de importarlo: ``sed -E 's/DEFINER=`[^`]+`@`[^`]+`/DEFINER=`usuario`@`%`/g;
  s/SQL SECURITY DEFINER/SQL SECURITY INVOKER/g' base.sql > base_servidor.sql``. **Adentro del contenedor no hace falta porque se conecta
...
> en `CLAUDE.md`, sección *Emitir y declarar son DOS pasos*.
