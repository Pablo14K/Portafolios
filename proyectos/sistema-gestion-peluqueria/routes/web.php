...
Route::middleware(['sesion', 'personal'])->group(function () {
    ...
    Route::prefix('citas')->name('citas.')->group(function () {
        ...
        Route::middleware('modulo:citas.agenda')->group(function () {
            ...
            Route::post('reasignar-una', [CitasController::class, 'reasignarUna'])
                ->middleware('admin')->name('reasignar.una');
    ...
    Route::prefix('seguridad')->name('seguridad.')->group(function () {
        ...
        // **Ajustes: cómo se presenta y cómo se ve el SISTEMA** —uno solo, no
        // uno por local—. Hasta la 7.124.0 vivía arriba de la lista de
        // Sucursales y la sobrecargaba; es su propio submódulo por pedido del
        // usuario. **Un solo POST para todo**: identidad, apariencia, texto y
        // datos fiscales — con dos hay que guardar dos veces para dejar la
        // pantalla como uno la quiere.
        Route::middleware('modulo:configuracion.ajustes')->group(function () {
            Route::get('ajustes', [ConfiguracionController::class, 'ajustes'])->name('ajustes');
            Route::post('ajustes', [ConfiguracionController::class, 'ajustesGuardar'])->name('ajustes.guardar');
            Route::post('ajustes/restablecer', [ConfiguracionController::class, 'ajustesRestablecer'])->name('ajustes.restablecer');
            // El logo se quita con su propio POST: un formulario no se anida
            // adentro de otro, así que el botón lo alcanza con `form=`.
            Route::post('ajustes/logo/quitar', [ConfiguracionController::class, 'ajustesLogoQuitar'])->name('ajustes.logo.quitar');
        });
    ...
    Route::prefix('inventario')->name('inventario.')->group(function () {
        ...
        Route::middleware('modulo:inventario.compras')->group(function () {
            ...
            // **La nota de crédito del proveedor** (7.127.0): una compra es su
            // factura, y una factura puede tener su nota. Se registra desde la
            // ficha de la compra, con el documento adjunto, y se anula — nunca
            // se borra.
            Route::post('compras/nota-credito', [InventarioController::class, 'compraNotaCredito'])
                ->name('compra.nota_credito');
            Route::post('compras/nota-credito/anular', [InventarioController::class, 'compraNotaAnular'])
                ->name('compra.nota_credito.anular');
            Route::get('compras/nota-credito/archivo', [InventarioController::class, 'compraNotaArchivo'])
                ->name('compra.nota_credito.archivo');
        });
    });
