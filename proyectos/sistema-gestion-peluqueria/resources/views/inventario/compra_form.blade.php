            ...
            <div class="sgp-panel">
                <form method="post" action="{{ route('inventario.compra.guardar') }}" id="formCompra">
                    ...
                    {{-- Las cuotas sólo tienen sentido a crédito, así que el
                         ...
                         que viene. --}}
                    <div id="bloqueCuotas" class="d-none mb-3 p-3 rounded" style="border:1px solid var(--borde)">
                    ...
                    {{-- **El subtotal necesita su propia columna.** Con `col-md-1`
                         ...
                         uno solo. **Y el IVA la suya** (7.127.0): toda factura
                         paraguaya lo desglosa por renglón —exentas, 5 %, 10 %— y la
                         compra es el reflejo de esa factura. 3+2+2+1+2+2 = 12. --}}
                    ...
                    <div id="filasCompra">
                        @for ($i = 0; $i < $cuantas; $i++)
                            <div class="row g-2 mb-2 filaCompra">
                                ...
                                <div class="col-md-1">
                                    {{-- **El IVA del renglón, como lo dice la factura.** Se
                                         propone el del producto al elegirlo y se puede
                                         cambiar: lo que se guarda es lo que se facturó ese
                                         día, y el producto puede cambiar de tasa después —
                                         el mismo criterio que `detalle_factura.tasa_iva`. --}}
                                    <select class="form-select form-select-sm ivaProd" name="tasa_iva[]"
                                            aria-label="IVA del renglón">
                                        @foreach ([10 => '10 %', 5 => '5 %', 0 => 'Ex.'] as $tasa => $rotIva)
                                            <option value="{{ $tasa }}"
                                                @selected((int) ($vIva[$i] ?? 10) === $tasa)>{{ $rotIva }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-2">
                    ...
                    {{-- **El total no estaba en ningún lado.** Se cargaban las
                         ...
                         descubría al pagar. --}}
                    {{-- **Y el descuento del comprobante** (7.127.0). La compra es el
                         reflejo de la factura del proveedor, y una factura puede traer
                         un descuento sobre el total: sin dónde cargarlo, el total del
                         sistema no coincidía con el del papel y la deuda quedaba de
                         más. Es UN monto del comprobante, no uno por renglón, que es
                         como lo trae casi todo papel. --}}
                    <div class="d-flex justify-content-end align-items-center gap-3 mt-2 flex-wrap"
                         style="border-top:1px solid var(--borde);padding-top:.6rem">
                        <span class="text-muted-warm" id="compraLineas">0 renglones</span>
                        <span class="text-muted-warm">Renglones: <span id="compraBruto">Gs. 0</span></span>
                        <label class="d-flex align-items-center gap-2 mb-0" for="compraDescuento">
                            <span class="text-muted-warm">Descuento</span>
                            <span class="input-group input-group-sm" style="width:150px">
                                <span class="input-group-text">{{ config('sgp.moneda') }}</span>
                                <input class="form-control input-miles" id="compraDescuento" name="descuento"
                                       data-min="0" value="{{ old('descuento') }}" placeholder="0">
                            </span>
                        </label>
                        <x-ayuda>El descuento que figura en la factura del proveedor, sobre el total. Vacío es sin descuento.</x-ayuda>
                        <strong>Total: <span class="txt-acento" id="compraTotal">Gs. 0</span></strong>
                    </div>
                                            ...
                                            <tbody>
                                                @foreach ($productos as $p)
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td class="text-end">
                                                            <button type="button" class="btn btn-sm btn-acento sgp-elegir-prod"
                                                                    data-bs-dismiss="modal"
                                                                    data-id="{{ $p->id_producto }}"
                                                                    data-nombre="{{ $p->nombre }}"
                                                                    data-precio="{{ $p->ultimo_precio !== null ? (int) $p->ultimo_precio : '' }}"
                                                                    data-iva="{{ (int) $p->tasa_iva }}"
...
(function () {
    ...
    function totalCompra() {
        ...
        // Las cuotas reparten lo que se DEBE, que es el total con el
        // descuento del comprobante ya aplicado.
        var d = document.getElementById('compraDescuento');
        var dn = parseFloat((d && d.value || '0').replace(/\./g, '').replace(',', '.')) || 0;
        return Math.max(t - dn, 0);
    }
...
(function () {
    ...
    function recalcular() {
        ...
        // **El total es los renglones menos el descuento del comprobante.**
        // Un descuento mayor que los renglones no es un descuento: se muestra
        // en rojo y el servidor lo rechaza.
        var desc = aNumero((document.getElementById('compraDescuento') || {}).value);
        var t = document.getElementById('compraTotal');
        var b = document.getElementById('compraBruto');
        var l = document.getElementById('compraLineas');
        if (b) { b.textContent = 'Gs. ' + miles(total); }
        if (t) {
            t.textContent = 'Gs. ' + miles(Math.max(total - desc, 0));
            t.classList.toggle('txt-no', desc > total && total > 0);
        }
        if (l) { l.textContent = renglones + (renglones === 1 ? ' renglón' : ' renglones'); }
    }

    var campoDesc = document.getElementById('compraDescuento');
    if (campoDesc) { campoDesc.addEventListener('input', recalcular); }
    ...
    function enganchar(fila) {
        ...
        function resolver() {
            ...
            var antes = oculto.value;
            oculto.value = op ? op.dataset.id : 0;

            // **El IVA del producto se propone al cambiar de producto**, no en
            // cada tecla: `input` dispara por letra, y pisar el combo cada vez
            // borraría lo que la persona ya corrigió mirando la factura.
            var iva = fila.querySelector('.ivaProd');
            if (op && iva && op.dataset.iva !== undefined && String(oculto.value) !== String(antes)) {
                iva.value = op.dataset.iva;
            }
    ...
    document.getElementById('masFilas').addEventListener('click', function () {
        ...
        copia.querySelectorAll('select.ivaProd').forEach(function (s) { s.value = '10'; });
    ...
    document.getElementById('filasCompra').addEventListener('click', function (ev) {
        ...
        if (cont.querySelectorAll('.filaCompra').length > 1) {
            ...
            fila.querySelectorAll('select.ivaProd').forEach(function (s) { s.value = '10'; });
        }
        recalcular();
    });
    ...
    document.querySelectorAll('.sgp-elegir-prod').forEach(function (b) {
        b.addEventListener('click', function () {
            ...
            var iva = fila.querySelector('.ivaProd');
            if (iva && b.dataset.iva !== undefined) { iva.value = b.dataset.iva; }
