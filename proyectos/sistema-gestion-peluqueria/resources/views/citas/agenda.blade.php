...
@section('contenido')
    ...
    <div class="sgp-panel mb-3">
        <form method="get" class="d-flex gap-2 align-items-end flex-wrap">
            ...
            <button class="btn btn-sm btn-acento"><i class="bi bi-calendar-week"></i> Ver</button>
    ...
    @if ($puedeCobrar && ! $caja)
        ...
        <div class="alert alert-warning">
            La agenda se puede consultar aunque la caja esté cerrada.
            @if ($hayCuentas)
                Para cobrar, la caja está cerrada: se puede registrar lo que llega <strong>por transferencia</strong>
                —entra a la cuenta bancaria—, pero para cobrar en efectivo hay que abrirla primero.
            @else
                Para cobrar, la caja está cerrada y no hay ninguna cuenta bancaria cargada. Para cobrar una seña
                        ...
                        <tr>
                            ...
                            <td class="text-muted-warm" data-label="Servicios">
                                ...
                                @else
                                    ...
                                    <span class="txt-acento">{{ $c->mis_servicios }}</span>
                            ...
                            <td class="text-end sgp-movil-acciones" style="white-space:nowrap">
                                ...
                                @php
                                    ...
                                    // **Y de la HORA** (7.127.0, reportado: «aparecen botones que
                                    // no deberían mostrarse antes, como registrar atención»). Un
                                    // día tiene treinta citas y a las 9 de la mañana la de las 18
                                    // no se atiende ni se pone en proceso: el servidor lo rechaza
                                    // desde la 7.82.0 y la fila lo ofrecía igual. Atender y «En
                                    // proceso» aparecen desde `$minutosAntes` antes; «Ausente»,
                                    // recién cuando la hora pasó — antes, quien no vino todavía
                                    // puede venir. Cancelar y reprogramar siguen desde siempre.
                                    $sgpFaltan = (strtotime((string) $c->fecha_hora) - strtotime($ahora)) / 60;
                                    $sgpYaToca = $esHoy && $sgpFaltan <= $minutosAntes;
                                    $sgpYaPaso = $esHoy && $sgpFaltan <= 0;
                                    ...
                                    $sgpTraba = $sgpAbierta && $sgpYaToca && ! $enCurso && ! ($c->fichaje_ok ?? true)
                                ...
                                @if ($sgpAbierta)
                                    ...
                                    @if ($sgpYaToca && ($c->fichaje_ok ?? true)
                                ...
                                @elseif ($c->estado === 'Atendida')
                                    ...
                                    @else
                                        ...
                                        @elseif ($puedeCobrar && ($caja || $hayCuentas) && $sgpFalta > 0.5)
                                            ...
                                            <button class="btn btn-sm btn-acento" type="button" title="Cobrar esta atención"
                                                    data-bs-toggle="modal" data-bs-target="#modalSena{{ $c->id_cita }}">
            ...
            <div class="modal-dialog modal-lg modal-dialog-scrollable">
                <div class="modal-content">
                    <div class="modal-header">
                        <div>
                            <h5 class="modal-title" style="font-size:1rem">
                                <i class="bi bi-card-text txt-acento"></i>
                            ...
                            <section class="sgp-cita-bloque">
                                ...
                                <dl>
                                    ...
                                    @if ($sgpTotal > 0)
                                        <dt>Total</dt>
                                        <dd>
                                            <strong class="txt-acento" style="font-size:1.05rem">{{ money($sgpTotal) }}</strong>
                                ...
                                <section class="sgp-cita-bloque sgp-cita-ancho">
                                    ...
                                    <dl class="sgp-cita-cobros">
                                        ...
                                        @if ($c->id_factura)
                                            <dt>Comprobante</dt>
                                            <dd>
                                                <a class="link-acento" href="{{ route('facturacion.factura_ver', ['id' => $c->id_factura]) }}">{{ $c->nro_comprobante }}</a>
                                                @if ((float) $c->saldo > 0.01)
                                        ...
                                        @elseif ($sgpTotal > 0 && $c->estado === 'Atendida')
                                            ...
                                            <dd>
                                                @if ($sgpFalta > 0.5)
                                                    <strong class="txt-acento">{{ money($sgpFalta) }}</strong>
                                                @else
                                        ...
                                        @if ($sgpCuenta)
                                            @foreach ($sgpCuenta as $sgpP => $sgpX)
                                                ...
                                                <dd>
                                                    ...
                                                    @if ($sgpX['id_factura'])
                                                        · <a class="link-acento" href="{{ route('facturacion.factura_ver', ['id' => $sgpX['id_factura']]) }}">{{ $sgpX['nro'] }}</a>
                    ...
                    <div class="modal-footer">
                        ...
                        @endif
                        <button type="button" class="btn btn-acento" data-bs-dismiss="modal">Cerrar</button>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="modal-content">
                    <form method="post" action="{{ route('citas.reprogramar') }}">
                        ...
                        <div class="modal-footer">
                            ...
                            <button class="btn btn-acento" id="btnRepro{{ $c->id_cita }}" disabled>Reprogramar</button>
                        </div>
                    </form>
                </div>
            </div>
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <form method="post" action="{{ route('citas.reasignar.una') }}">
                            ...
                            <div class="modal-footer">
                                <button type="button" class="btn btn-outline-neutro" data-bs-dismiss="modal">Cancelar</button>
                                <button class="btn btn-acento"><i class="bi bi-check-lg"></i> Cambiar</button>
                            </div>
                        </form>
                    </div>
                </div>
                        ...
                        <form method="post" action="{{ route('facturacion.sena') }}">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                @if ($pide > 0 && $c->estado !== 'Atendida' && ! $enCursoCobro)
                                                    ...
                                                    <th class="text-end txt-acento">{{ money($pide) }}</th>
                                                @else
                                                    <th>A cobrar</th>
                                                    <th class="text-end txt-acento" data-cobro-falta>{{ money($falta) }}</th>
                                                @endif
                                            </tr>
                                        </tbody>
                                    </table>
                            ...
                            @if ($c->id_solicitud)
                                ...
                                <div class="mb-2">
                                    ...
                                    <div class="val acento" style="font-size:1.25rem">{{ money($sugerido) }}</div>
