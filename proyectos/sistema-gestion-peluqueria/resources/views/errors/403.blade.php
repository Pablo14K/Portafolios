...
<div class="container" style="max-width:520px;margin-top:12vh">
    <div class="sgp-panel text-center">
        <div style="font-size:2.2rem;color:var(--acento)">&#128274;</div>
        ...
        @unless ($enBucle)
            <a class="btn btn-acento" href="{{ $url }}">Volver al inicio</a>
        @endunless
        ...
        @if (session('uid'))
            ...
            <form method="post" action="{{ route('salir') }}" class="d-inline">
                @csrf
                <button class="btn {{ $enBucle ? 'btn-acento' : 'btn-outline-neutro' }}">
