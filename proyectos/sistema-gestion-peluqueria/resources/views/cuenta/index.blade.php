...
@section('contenido')
    ...
    @if ($pendiente)
        <div class="alert alert-warning d-flex justify-content-between align-items-center flex-wrap gap-2">
            ...
            <a class="btn btn-sm btn-acento" href="{{ route('cuenta.password_confirmar') }}">Confirmarlo</a>
        </div>
    @endif
                ...
                <div class="d-flex align-items-center gap-3 mb-3">
                    ...
                    <div class="flex-grow-1">
                        <form method="post" action="{{ route('cuenta.foto') }}" enctype="multipart/form-data"
                            ...
                            <button class="btn btn-sm btn-acento"><i class="bi bi-upload"></i> Subir</button>
                        </form>
                    ...
                    <tbody>
                        ...
                        @if ($sucursalActiva)
                            <tr><td class="text-muted-warm">Sucursal</td>
                                <td><strong class="txt-acento">{{ $sucursalActiva }}</strong></td></tr>
                        @endif
                    </tbody>
                </table>
            </div>
                ...
                <form method="post" action="{{ route('cuenta.tema') }}" class="d-flex gap-2 flex-wrap">
                    ...
                    @foreach (\App\Servicios\Sesion::TEMAS as $clave => $etiqueta)
                        <button name="tema" value="{{ $clave }}"
                                class="btn {{ $tema === $clave ? 'btn-acento' : 'btn-outline-neutro' }}">
            ...
            <div class="sgp-panel">
                ...
                @else
                    <form method="post" action="{{ route('cuenta.password') }}">
                        ...
                        <button class="btn btn-acento"><i class="bi bi-envelope"></i> Mandarme el código</button>
                    </form>
                @endif
            </div>
        </div>
