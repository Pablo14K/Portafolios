...
@section('contenido')
    ...
    @if (! $tipos)
        <div class="alert alert-danger">
            ...
            @if (\App\Servicios\Permisos::puede('facturacion.timbrados'))
                <a class="link-acento" href="{{ route('facturacion.timbrados') }}">Cargá uno acá</a>.
            @else
        ...
        <div class="alert alert-warning">
            ...
            **sin su propio timbrado no se puede numerar**, así que no aparece en la
            ...
            <div class="mt-1" style="font-size:.86rem">
                ...
                @if (\App\Servicios\Permisos::puede('facturacion.timbrados'))
                    <a class="link-acento" href="{{ route('facturacion.timbrados') }}">Cargá el
    ...
    @if (! empty($timbradoAjeno))
        <div class="alert alert-warning">
            ...
            no al de acá.
            @if (\App\Servicios\Permisos::puede('facturacion.timbrados'))
                <a class="link-acento" href="{{ route('facturacion.timbrados') }}">Cargale un timbrado a este local</a>
                        ...
                        <tr @class(['sgp-fila-elegida' => $elegida])>
                            ...
                            <td class="text-end">
                                ...
                                @if ((float) ($c->canjeado ?? 0) > 0 || $desc > 0)
                                    ...
                                    @endif
                                    <strong class="txt-acento">{{ money($base - $desc) }}</strong>
                                @else
                                    {{ money($c->total) }}
                                @endif
                            </td>
