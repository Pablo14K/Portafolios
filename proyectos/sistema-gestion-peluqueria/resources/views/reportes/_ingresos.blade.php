    ...
    <div class="row g-3">
        <div class="col-lg-5">
            <div class="sgp-panel h-100">
                <h2 class="sgp-form-titulo mb-2"><i class="bi bi-credit-card"></i> Por medio de pago</h2>
                ...
                @if ($devoluciones > 0)
                    <div class="sgp-suma-at mt-3">
                        <div class="sgp-suma-fila"><span>Cobrado</span><strong>{{ money($ingresos) }}</strong></div>
                        <div class="sgp-suma-fila"><span>Devuelto (notas de crédito)</span>
                            <strong class="txt-no">− {{ money($devoluciones) }}</strong></div>
                        <div class="sgp-suma-fila sgp-suma-total"><span>Ingreso neto</span>
                            <strong class="val acento">{{ money($ingresos - $devoluciones) }}</strong></div>
                    </div>
                @endif
            </div>
        </div>
        ...
        <div class="col-lg-7">
            <div class="sgp-panel h-100">
                <h2 class="sgp-form-titulo mb-2"><i class="bi bi-graph-up"></i> Cobrado día por día</h2>
                @if ($ingresoDia)
                    @foreach ($ingresoDia as $x)
                        @php $pct = $maxDia > 0 ? round((float) $x->total * 100 / $maxDia) : 0; @endphp
                        <div class="sgp-graf-fila">
                            <span class="sgp-graf-rot" style="width:86px">{{ fecha($x->dia, 'd/m/Y') }}</span>
                            <span class="sgp-graf-pista"><span class="sgp-graf-barra" style="width:{{ $pct }}%"></span></span>
                            <span class="sgp-graf-val sgp-graf-val-ancho">{{ money($x->total) }}</span>
                        </div>
                    @endforeach
                @else
                    <p class="sgp-sin-datos">Sin cobros en el período seleccionado.</p>
                @endif
            </div>
        </div>
        ...
        <div class="col-lg-6">
            <div class="sgp-panel h-100">
                <h2 class="sgp-form-titulo mb-2"><i class="bi bi-person-badge"></i> Facturado por profesional</h2>
                @if ($equipo)
                    @php $maxGen = 0; foreach ($equipo as $e) { $maxGen = max($maxGen, (float) $e->generado); } @endphp
                    @foreach ($equipo as $e)
                        @continue((float) $e->generado <= 0)
                        <div class="sgp-graf-fila">
                            <span class="sgp-graf-rot" style="width:150px">{{ $e->profesional }}</span>
                            <span class="sgp-graf-pista"><span class="sgp-graf-barra"
                                  style="width:{{ $maxGen > 0 ? round((float) $e->generado * 100 / $maxGen) : 0 }}%"></span></span>
                            <span class="sgp-graf-val sgp-graf-val-ancho">{{ money($e->generado) }}</span>
                        </div>
                    @endforeach
                @else
                    <p class="sgp-sin-datos">Sin atenciones en el período seleccionado.</p>
                @endif
            </div>
        </div>
        ...
        <div class="col-lg-6">
            <div class="sgp-panel h-100">
                <h2 class="sgp-form-titulo mb-2"><i class="bi bi-scissors"></i> Facturado por servicio</h2>
                @if ($servicios)
                    @php
                    ...
                    @foreach ($porIngreso as $s)
                        <div class="sgp-graf-fila">
                            <span class="sgp-graf-rot" style="width:150px">{{ $s->servicio }}</span>
                            <span class="sgp-graf-pista"><span class="sgp-graf-barra"
                                  style="width:{{ $maxIng > 0 ? round((float) $s->ingreso_generado * 100 / $maxIng) : 0 }}%"></span></span>
                            <span class="sgp-graf-val sgp-graf-val-ancho">{{ money($s->ingreso_generado) }}</span>
                        </div>
                    @endforeach
                @else
                    <p class="sgp-sin-datos">Sin servicios en el período seleccionado.</p>
                @endif
            </div>
        </div>
        ...
        @if ($ingresoSucursal && count($ingresoSucursal) > 1)
            <div class="col-12">
                <div class="sgp-panel">
                    <h2 class="sgp-form-titulo mb-2"><i class="bi bi-shop"></i> Cobrado por sucursal</h2>
                    @php $maxSuc = 0; foreach ($ingresoSucursal as $s) { $maxSuc = max($maxSuc, (float) $s->total); } @endphp
                    @foreach ($ingresoSucursal as $s)
                        <div class="sgp-graf-fila">
                            <span class="sgp-graf-rot" style="width:170px">{{ $s->sucursal }}</span>
                            <span class="sgp-graf-pista"><span class="sgp-graf-barra"
                                  style="width:{{ $maxSuc > 0 ? round((float) $s->total * 100 / $maxSuc) : 0 }}%"></span></span>
                            <span class="sgp-graf-val sgp-graf-val-ancho">{{ money($s->total) }}</span>
                        </div>
                    @endforeach
                </div>
            </div>
