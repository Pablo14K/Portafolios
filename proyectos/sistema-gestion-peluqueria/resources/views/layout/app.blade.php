...
<head>
    ...
    {{-- **Los colores que eligió el salón, escritos como variables.**

         Todo el sistema está pintado con `var(--…)`, así que redefinirlas acá
         alcanza para que cambien las treinta pantallas, los dos temas y los
         componentes de Bootstrap. **No hay un solo selector de componente**, que
         es la misma regla que gobierna el bloque del tema oscuro: si hiciera
         falta uno, es que algo se escribió con un color suelto.

         Lo que sí hay que escribir a mano son las variables que Bootstrap
         declara en RGB y no en hex —`--bs-*-rgb`, el anillo de foco—: no salen
         de un `var()`, así que sin estas líneas `.text-body` y el foco se
         quedarían con el verde agua mientras el resto cambia.

         El bloque sólo se dibuja si el salón tocó algo, y adentro cada parte
         sólo si cambió: un salón que eligió la letra grande y dejó el verde
         agua no tiene por qué recibir cuarenta variables con los valores de
         fábrica. De fábrica no se emite ni un byte de más. --}}
    @php
        $sgpTema = \App\Servicios\Config::tema();
        $sgpC = $sgpTema['tokens'];
        // **El mapa token → variable vive en `Tema`, no acá.** Lo leen tres
        // lugares —esta línea, la guardia que comprueba que el verde agua de
        // `Tema` siga siendo el de `app.css`, y quien agregue un token—, y
        // escrito en cada uno, renombrar una variable deja dos apuntando al
        // vacío sin que nada dé error.
        $sgpVars = function (array $mapa) use ($sgpC) {
            $out = '';
            foreach ($mapa as $token => $vars) {
                if (empty($sgpC[$token])) { continue; }
                foreach ($vars as $v) { $out .= $v . ':' . $sgpC[$token] . ';'; }
            }
            return $out;
        };
        // Las que Bootstrap declara en RGB y no en hex: no salen de un `var()`,
        // así que sin escribirlas `.text-body` y el anillo de foco se quedan
        // con el verde agua mientras el resto cambia.
        $sgpRgb = function (string $texto, string $acento) use ($sgpC) {
            $t = \App\Servicios\Tema::csv($sgpC[$texto] ?? '#0F4C43');
            $a = \App\Servicios\Tema::csv($sgpC[$acento] ?? '#1A6B5F');
            return '--bs-body-color-rgb:' . $t . ';--bs-primary-rgb:' . $a
                . ';--bs-link-color-rgb:' . $a . ';--bs-focus-ring-color:rgba(' . $a . ',.25);';
        };
    @endphp
    @if (\App\Servicios\Config::temaPersonalizado())
        <style>
            @if (\App\Servicios\Config::coloresPersonalizados())
                :root{ {!! $sgpVars(\App\Servicios\Tema::variables()) !!}{!! $sgpRgb('texto', 'acento') !!} }
                [data-tema="oscuro"]{ {!! $sgpVars(\App\Servicios\Tema::variablesOscuro()) !!}{!! $sgpRgb('texto_oscuro', 'acento_oscuro') !!} }
            @endif
            @if ($sgpTema['letra'] !== 'normal')
                {{-- **La letra grande escala la página entera y no un renglón.**
                     `app.css` no tiene una sola medida en píxeles —son 165 en
                     `rem`— así que mover la base del documento agranda texto,
                     botones, altos de fila y espacios juntos, en vez de dejar
                     un texto grande adentro de una caja del tamaño de antes. --}}
                html{font-size:{{ \App\Servicios\Tema::LETRAS[$sgpTema['letra']][1] }};}
            @endif
            @if ($sgpTema['fuente'] !== 'sistema')
                {{-- **El tipo de letra va en `body`, y con eso alcanza**: los
                     campos y los botones heredan la fuente —es el reboot de
                     Bootstrap— y ninguna pantalla declara la suya. Son pilas
                     del sistema: no se descarga ninguna fuente. Va sin escapar
                     porque es una constante de `Tema`, no algo que alguien
                     tipeó, y escapada las comillas de «Times New Roman» salen
                     como `&quot;`, que adentro de un `<style>` no es nada. --}}
                body{font-family:{!! \App\Servicios\Tema::FUENTES[$sgpTema['fuente']][1] !!};}
            @endif
        </style>
    @endif
</head>
                        ...
                        @foreach ($sgpAlertas as $sgpA)
                            <li>
                                <div class="sgp-alerta @unless ($sgpA['visto']) sin-ver @endunless">
                                    ...
                                    {{-- Un aviso puede traer su URL armada —la agenda de OTRO día,
                                         con la ventana de cobro abierta— y si no, la del catálogo --}}
                                    @if ($sgpUrlA = ($sgpA['url'] ?? null) ?: ($sgpA['ruta'] ? Navegacion::url($sgpA['ruta']) : null))
                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-acento" data-bs-dismiss="modal">Aceptar</button>
                        </div>
                    </div>
                </div>
            </div>
