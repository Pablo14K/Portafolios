...
@section('contenido')
    ...
    <div class="sgp-panel mb-3">
        <form method="get" class="d-flex gap-2 align-items-end flex-wrap">
            ...
            <button class="btn btn-sm btn-acento"><i class="bi bi-calendar-check"></i> Ver</button>
                                    ...
                                    @if ($porOtros || $mio)
                                        @if (! $cerrado && ! $f->hora_entrada
                                            ...
                                            <form method="post" action="{{ route('seguridad.asistencia.marcar') }}" class="d-inline">
                                                ...
                                                @endif
                                                <button class="btn btn-sm btn-acento"><i class="bi bi-box-arrow-in-right"></i>
                                        ...
                                        @elseif (! $cerrado && $f->hora_entrada && ! $f->hora_salida)
                                            <form method="post" action="{{ route('seguridad.asistencia.marcar') }}" class="d-inline">
                                                ...
                                                @endif
                                                <button class="btn btn-sm btn-acento"><i class="bi bi-box-arrow-right"></i> Marcar salida</button>
                                            </form>
                                        @endif
                    ...
                    <div class="modal-content">
                        <form method="post" action="{{ route('seguridad.asistencia.marcar') }}">
                            ...
                            <div class="modal-footer">
                                ...
                                <button class="btn btn-acento" name="accion" value="falta_sin">
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <form method="post" action="{{ route('seguridad.asistencia.marcar') }}">
                            ...
                            <div class="modal-footer">
                                <button type="button" class="btn btn-outline-neutro" data-bs-dismiss="modal">Cancelar</button>
                                <button class="btn btn-acento"><i class="bi bi-check-lg"></i> Justificar</button>
                            </div>
                        </form>
                    </div>
                </div>
    ...
    <div class="sgp-panel mt-3">
        ...
        <x-filtros :f="$fa" :ocultos="['fecha' => $fecha]" />
        ...
        @if ($rows)
            ...
            {{-- **Con paginación** (7.127.0): eran sesenta filas fijas, y la
                 sesenta y una no existía. La fecha de arriba se arrastra, que
                 si no pasar de página devolvía la lista del día a hoy. --}}
            <x-paginacion :pag="$pagA" :f="$fa" :ocultos="['fecha' => $fecha]" />
        @else
            <div class="sgp-vacio">
