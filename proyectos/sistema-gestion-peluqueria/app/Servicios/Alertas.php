...
class Alertas
{
    ...
    public static function todas(): array
    {
        ...
        foreach (['senasPorConfirmar', 'cajasAbiertasDeMas', 'faltaStock'] as $punto) {
            try {
                self::$punto();
            } catch (Throwable) {
                continue;
    ...
    /**
     * Una clienta dice que transfirió la seña, y nadie la confirmó todavía.
     *
     * **Es lo primero de la bandeja, porque tiene reloj** (7.127.0, pedido del
     * usuario: «validar si llegó la seña para cobrarla — hacer que haya un
     * aviso»). La solicitud del portal no es plata que entró: es la clienta
     * avisando que pagó. Hasta que alguien mire el banco y la confirme, la
     * cita sigue **sin confirmar** y `cancelarSenasVencidas()` la suelta al
     * cumplirse el plazo — con la clienta convencida de que ya está. La
     * solicitud se veía sólo en la fila de la agenda de ESE día, así que la
     * de la cita de la semana que viene no la veía nadie.
     *
     * Va a quien cobra (`facturacion.cobros`), que es quien confirma, y del
     * local de la cita. El enlace abre la agenda en el día de la cita **con la
     * ventana de cobro ya abierta** (`?cobrar=`), que es lo que hace que
     * confirmar sea un clic y no encontrar la fila.
     *
     * La identidad es la SOLICITUD: confirmada o rechazada, el renglón se va
     * solo; una segunda solicitud de la misma cita es otro aviso.
     */
    private static function senasPorConfirmar(): void
    {
        $par = [];
        $filtro = Sucursales::filtro('c', $par);
        $horas = max(1, (int) config('sgp.agenda.sena_horas', 24));

        $filas = DB::select(
            "SELECT ss.id_solicitud, ss.id_cita, ss.monto, ss.comprobante, ss.fecha_solicitud,
                    c.fecha_hora, CONCAT(pe.nombre, ' ', pe.apellido) AS cliente
               FROM sena_solicitud ss
               JOIN cita c ON c.id_cita = ss.id_cita
               JOIN cliente cl ON cl.id_cliente = c.id_cliente
               JOIN persona pe ON pe.id_persona = cl.id_persona
               JOIN estado_cita ec ON ec.id_estado_cita = c.id_estado_cita
              WHERE ss.id_cobro IS NULL AND ss.rechazada_en IS NULL
                AND ec.bloquea_agenda = 1
                $filtro
              ORDER BY c.fecha_hora", $par
        );

        foreach ($filas as $s) {
            self::$puntos[] = [
                'nivel' => 'SENA',
                'que' => $s->cliente . ' avisó que transfirió ' . money($s->monto) . ' de seña por su cita del '
                    . fecha($s->fecha_hora, 'd/m') . ' a las ' . fecha($s->fecha_hora, 'H:i')
                    . ($s->comprobante ? ', con el comprobante adjunto' : ', sin comprobante adjunto')
                    . '. Hay que comprobar que la plata llegó y confirmarla: hasta entonces la cita sigue '
                    . 'sin confirmar, y a las ' . $horas . ' horas se suelta el horario.',
                'donde' => 'Citas → Agenda',
                'ruta' => 'citas.agenda',
                // **Con el día de la cita y la ventana de cobro abierta**: el
                // catálogo de rutas sólo sabe armar la agenda de hoy.
                'url' => route('citas.agenda', ['dia' => fecha($s->fecha_hora, 'Y-m-d'), 'cobrar' => (int) $s->id_cita]),
                'permiso' => 'facturacion.cobros',
                'clave' => 'sena:' . (int) $s->id_solicitud,
            ];
        }
    }
