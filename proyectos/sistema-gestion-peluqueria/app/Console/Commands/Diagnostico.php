...
class Diagnostico extends Command
{
    ...
    private const ESPERADO = ['PROCEDURE' => 22, 'FUNCTION' => 51, 'trigger' => 17, 'vista' => 18];
        ...
        foreach ($definidores as $d) {
            ...
            if ($existe === true) {
                ...
                $this->mal($detalle . ': ese usuario NO existe en este servidor. Todo lo que las llame '
                    . 'va a contestar error 1449. Importá el .sql con el usuario que lo va a usar, o '
                    . 'reescribí los DEFINER antes: sed -E "s/DEFINER=`[^`]+`@`[^`]+`/DEFINER=`'
                    . strtok($usuario, '@') . '`@`%`/g; s/SQL SECURITY DEFINER/SQL SECURITY INVOKER/g"');
                $problemas++;
            } else {
