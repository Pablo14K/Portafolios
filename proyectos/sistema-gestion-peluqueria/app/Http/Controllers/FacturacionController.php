...
class FacturacionController extends Controller
{
    ...
    public function sena(Request $request): RedirectResponse
    {
        ...
        try {
            ...
            // **Y a la clienta le llega el recibo con la confirmación** (7.127.0).
            // Va después de registrar la seña y no atado a eso: si el correo
            // falla, la seña ya entró — y el aviso queda en la cola igual. El
            // mensaje dice si salió y a dónde, porque para ella «confirmada»
            // significa que le llegó.
            $recibo = Sena::mandarRecibo($idCita, $cobrados);
            flash('Seña de ' . money($monto) . ' registrada para ' . $cita->cliente
                . '. Se va a descontar sola del total cuando se facture la cita. '
                . ($recibo['ok']
                    ? 'Le mandamos el recibo y la confirmación a ' . $recibo['email'] . '.'
                    : 'El recibo no se le pudo mandar: ' . $recibo['motivo'] . '.'),
                $recibo['ok'] ? 'success' : 'warning');
        } catch (Throwable $ex) {
    ...
    public function movimientoCaja(Request $request): RedirectResponse
    {
        ...
        if ($request->hasFile('archivo')) {
            $archivo = Respaldo::guardar($request->file('archivo'), 'mov');
            if ($archivo === false) {
    ...
    public function proveedores(): View
    {
        ...
        foreach ($cuentas as $c) {
            ...
            // **Lo que el pago tiene que mostrar de la factura** (7.127.0):
            // las cuotas con lo que le falta a cada una —la compra a crédito
            // decía sólo el total, y la persona pagaba el total de una compra
            // en tres cuotas—, y el IVA que viene incluido en el precio.
            $cuotasPorCompra[(int) $c->id_compra] = (int) $c->cuotas > 0 ? Compras::cuotas((int) $c->id_compra) : [];
            $ivaPorCompra[(int) $c->id_compra] = DB::selectOne(
                'SELECT * FROM vw_compra_impuestos WHERE id_compra = ?', [(int) $c->id_compra]);
        }
        ...
        return view('facturacion.proveedores', [
            ...
            'cuotasPorCompra' => $cuotasPorCompra,
            'ivaPorCompra' => $ivaPorCompra,
