...
/**
 * La mitad del módulo Seguridad que trata del sistema: ajustes, sucursales,
 * roles, contacto y auditoría. La otra mitad —usuarios, turnos, comisiones y
...
class ConfiguracionController extends Controller
{
    ...
    /**
     * Cómo se presenta y cómo se ve el sistema: la identidad, la apariencia,
     * el texto y los datos fiscales.
     *
     * **Es UNO para todo el sistema, no uno por sucursal**: la clienta entra
     * por un único portal y ve una sola marca, y quien atiende ve la misma
     * trabaje donde trabaje — el mismo criterio que el Centro de Ayuda y
     * Soporte. Hasta la 7.124.0 vivía arriba de la lista de Sucursales, y la
     * sobrecargaba: es su propia pantalla por pedido del usuario.
     */
    public function ajustes(): View
    {
        return view('seguridad.ajustes', [
            'nombreSalon' => Config::nombreSalon(),
            'logo' => Config::logo(),
            'actividad' => Config::actividad(),
            'emailFiscal' => Config::email(),
            // La apariencia: el color elegido, la paleta que salió de él, el
            // tamaño y el tipo de letra. El catálogo y las reglas viajan para
            // que la vista previa pueda derivar en el navegador sin volver a
            // preguntar — ver `App\Servicios\Tema`.
            'tema' => Config::tema(),
            'catalogo' => Tema::catalogo(),
            'reglas' => Tema::reglas(),
            'personalizado' => Config::temaPersonalizado(),
        ]);
    }
    ...
    /**
     * Guarda los ajustes: identidad, apariencia, texto y datos fiscales.
     *
     * **Es UN solo formulario y un solo botón**, como la maqueta que dio el
     * usuario. Antes eran dos POST distintos —la identidad por un lado y los
     * colores por otro— y eso obliga a guardar dos veces para dejar la pantalla
     * como uno la quiere, con la mitad del cambio aplicada en el medio.
     *
     * **El logo se ve en el ingreso y en la barra de arriba, o sea antes y
     * después de entrar**, así que un archivo roto acá rompe la pantalla desde
     * la que se arregla. Por eso las tres defensas: se comprueba que sea una
     * imagen de verdad (`getimagesize`, no la extensión que diga el nombre), se
     * limita el tamaño, y **el archivo se escribe antes de tocar la base** — si
     * falla la escritura, la configuración queda como estaba.
     *
     ...
     */
    public function ajustesGuardar(Request $request): RedirectResponse
    {
        $volver = redirect()->route('seguridad.ajustes');
        ...
        // ---------- La apariencia y el texto ----------
        //
        // **El color es UNO, y de él sale el resto.** El catálogo le pone
        // nombre a los que ya están elegidos; «propio» es el que el salón saca
        // del selector del navegador. Un valor inventado en el POST cae en el
        // verde agua en vez de rechazar el guardado entero: el nombre y el logo
        // que vienen en el mismo formulario no tienen por qué perderse por eso.
        // Lo mismo con la letra: un tamaño o un tipo que no existe cae en el
        // de fábrica, que es lo que `Config::guardarTema()` hace sola.
        $catalogo = Tema::catalogo();
        $paleta = (string) $request->input('paleta', '');
        $primario = isset($catalogo[$paleta])
            ? $catalogo[$paleta][1]
            : Tema::normalizar((string) $request->input('color_primario', ''));
        if (! isset($catalogo[$paleta])) {
            $paleta = '';
        }

        $letra = (string) $request->input('letra', 'normal');
        $fuente = (string) $request->input('fuente', 'sistema');

        if (! Config::guardarTema($primario, $paleta ?: null, $letra, $fuente)) {
            flash('Se guardó el nombre, pero no la apariencia. El detalle quedó registrado.', 'error');
        ...
        Auditoria::registrar('MODIFICACION', 'Configuracion', 'configuracion', 1,
            'Ajustes del sistema: «' . $nombre . '»' . ($guardar ? ' + logo nuevo' : '')
            . ' · color ' . ($paleta !== '' ? $catalogo[$paleta][0] : ($primario ?: Tema::PRIMARIO))
            . ' · letra ' . $letra . ' / ' . $fuente);

        flash('Listo. Los ajustes se ven en todas las pantallas, para todo el equipo y para las clientas.');

        return $volver;
    }

    /** Saca el logo y vuelve a la tijera de siempre. */
    public function ajustesLogoQuitar(): RedirectResponse
    {
        ...
        return redirect()->route('seguridad.ajustes');
    }

    /**
     * Vuelve al verde agua de fábrica.
     *
     * Se BORRA lo guardado en vez de reescribir la paleta con los valores de
     * hoy: así el salón que restablece hereda la identidad que traiga la
     * versión que esté corriendo, y no una foto de la de antes.
     */
    public function ajustesRestablecer(): RedirectResponse
    {
        if (! Config::restablecerTema()) {
            flash('No se pudo restablecer. El detalle quedó registrado.', 'error');
        ...
        Config::olvidar();
        Auditoria::registrar('MODIFICACION', 'Configuracion', 'configuracion', 1,
            'Apariencia restablecida a la de fábrica');
        flash('Listo: vuelve el verde agua del sistema, con la letra de fábrica.');
