...
class PersonalController extends Controller
{
    ...
    public function asistencia(Request $request): View
    {
        ...
        // **Los últimos registros paginan** (7.127.0). Cortaban con `LIMIT 60`
        // sin decirlo, que es exactamente lo que el prototipo de listado existe
        // para evitar: a los tres meses de operación la fila 61 dejaba de
        // existir para quien la buscaba, y como el corte no se ve tampoco
        // había forma de sospecharlo. El `WHERE` se arma una vez y lo comparten
        // el `COUNT(*)` y la página, para que el «de N» del pie diga la verdad.
        $desdeA = 'FROM asistencia a
                   JOIN turno_laboral t ON t.id_turno = a.id_turno
                   JOIN usuario u       ON u.id_usuario = a.id_usuario
                   JOIN persona pe      ON pe.id_persona = u.id_persona
                  ' . ($wa ? 'WHERE ' . implode(' AND ', $wa) : '');
        $pagA = Listado::paginacion((int) DB::scalar("SELECT COUNT(*) $desdeA", $pa), 25);
        ...
        return view('seguridad.asistencia', [
            ...
            'rows' => DB::select(
                "SELECT a.*, t.nombre AS turno, t.hora_inicio, t.hora_fin,
                        CONCAT(pe.nombre,' ',pe.apellido) AS profesional
                  $desdeA
                  ORDER BY a.fecha DESC, t.hora_inicio DESC
                  LIMIT {$pagA['porPagina']} OFFSET {$pagA['offset']}", $pa
            ),
            'fecha' => $fecha,
            'dia' => $dia,
            'dias' => self::DIAS,
